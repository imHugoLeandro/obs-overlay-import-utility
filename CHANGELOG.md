...
## Unreleased
- Both import methods accept a **ZIP archive** in place of the usual input: Import OBS Scene Collection File gains a `ZIP…` browse button and extracts a chosen `.zip` beside itself into a unique folder before scanning for the OBS export; Import Streamlabs Scene File accepts a `.zip` wrapper containing exactly one `.overlay`, which is extracted first and then imported. ZIP redirects use the same hardened validation as `.overlay` packages (traversal, links, duplicates, sizes, ratios, free space).
