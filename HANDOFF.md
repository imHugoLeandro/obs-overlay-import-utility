...
- Branch: `main`
- 2026-09-03 (dev): Both import methods accept a ZIP redirect — the single **Import OBS Scene Collection File** Browse… button picks a normal folder or a `.zip` (safely extracted beside itself into a unique folder before scanning for the OBS export); **Import Streamlabs Scene File** accepts a `.zip` wrapper containing exactly one `.overlay` (extracted first, then imported). Shared `streamlabs.extract_zip_archive()` reuses the hardened archive validation (traversal, links, duplicates, sizes, ratios, free space) with a generic "ZIP archive" label; `_safe_archive_members`/`_archive_member_path` now take a `label` parameter.
